...
COMPILE_STATUS="pass"
dotnet build "./BenchmarkDotNet.sln" -p:TreatWarningsAsErrors=false > /tmp/compile_stdout.log 2> /tmp/compile_stderr.log || {
  COMPILE_STATUS="fail"
}
...
if [ "$COMPILE_STATUS" = "pass" ] && [ "$PATCH_STATUS" = "pass" ]; then
  dotnet build "./BenchmarkDotNet.sln" -p:TreatWarningsAsErrors=false > /tmp/rebuild_stdout.log 2> /tmp/rebuild_stderr.log || {
    REBUILD_STATUS="fail"
    COMPILE_STATUS="fail"
  }
